...
class JunitTestStudent {
	...
	public static final String TEST_STUDENT_STATUS1 = "HOLD";
	...
	//Story 1: Test for adding a new student function 
	@Test
	...
	//Story 2: Test for adding a hold to a student method
	@Test
	public void addHoldtoStudent()  throws Exception {
		...
		//assumes that will return student to ensure a student was found with given input
		...
		//create a DTO to get json response from server
		StudentDTO studentDTO = fromJsonString(response.getContentAsString(), StudentDTO.class);
		
		//compares the data actually appears in the json response
		assertEquals(TEST_STUDENT_ID, studentDTO.student_id);
		assertEquals(TEST_STUDENT_NAME, studentDTO.name);
		assertEquals(TEST_STUDENT_EMAIL, studentDTO.email);
	...
	//Story 3: Test for releasing a hold from a student
	@Test
	public void releaseHold() throws Exception{
		...
		//Assuming that the user exits, findByEmail will return the student of the inputed test email
		given(studentRepository.findByEmail(TEST_STUDENT_EMAIL)).willReturn(student);
		//Assuming that the changes were saved
		given(studentRepository.save(any(Student.class))).willReturn(student);
		
		//mock request for testing the function
		response = mvc.perform(
				MockMvcRequestBuilders
				.put("/student/releasehold/{email}", TEST_STUDENT_EMAIL))
				.andReturn().getResponse();
		...
		//compares the test data to the actual response data to ensure it is actually inputed
		assertEquals(TEST_STUDENT_ID, studentDTO.student_id);
		assertEquals(TEST_STUDENT_NAME, studentDTO.name);
		assertEquals(TEST_STUDENT_EMAIL, studentDTO.email);
		...
		System.out.println("Removed hold from user: " + studentDTO);
	}
